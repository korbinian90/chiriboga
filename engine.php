...
<html>
	...
	<button class="fullscreen-button" onclick="document.getElementById('body').requestFullscreen({ navigationUI: 'hide' });"></button>
		<div id="fps"></div>
		<!-- Modern HUD overlay: agenda score chip top-center, big "Continue"
		     CTA bottom-center, and a turn-change banner that fades in on
		     player flip. The existing PIXI floating counters (clicks /
		     credits / deck-size) handle per-side stats and are left
		     untouched — they're aesthetically integrated with the scene
		     and the user prefers them to redundant HTML chips.
		     Driven by UpdateModernHUD() in init.js, called from Render(). -->
		<div id="modern-hud">
			<div class="hud-score" id="hud-score" title="Agenda points (you / target)">
